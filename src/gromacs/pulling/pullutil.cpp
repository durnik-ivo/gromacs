...
static bool densmap_find_minimum(struct pull_t *pull, t_pbc *pbc, rvec x_min)
{
    ...
    if (minidx >= 0)
    {
        ...
        x_min[0] = (i + 0.5) / binfac[0];
        x_min[1] = (j + 0.5) / binfac[1];
        x_min[2] = 0;
        return true;
    }
    else
        ...
        for (int groupIndex = 0; groupIndex < coordParams.ngroup; groupIndex++)
        {
            for (int d = 0; d < DIM; d++)
            {
                if (coordParams.dim[d] &&
                    ...
                    coordParams.eGeom != epullgCYLDENSMDISO)
                {
